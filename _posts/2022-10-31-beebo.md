...
### Midi map key scale
Transposes each of the 12 keys up or down individually.
...
### Midi strum
When a cord goes in, the individual notes are delayed so that it sounds like strumming.

If multiple input notes are pressed in a time interval given by **Note collect timeout [ms]**, they are recognized as a chord.
