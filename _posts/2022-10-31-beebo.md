...
Is based on the _Resonestor_ mode of the _Parasite_ firmware for _Mutable Instruments Clouds_.
...
### Euclidean
The output is up to 4 tracks of _CvTriggers_ which can be used to create a drum pattern when connected to a drum sound source.
...
**T2** is open.
The outputs **T1 Output**/**X1 Output** and **T3 Output**/**X3 Output** work similar. Their appearances and values are in a certain way dependend of **T2**/**X2**.
...
#### Steps quantizer
Allows to set the scale which is used when **Smoothness** in the **X voltage** page is set to the right from the center.
...
- **Different shapes**: ouput 1 is the “normal” output which is shaped like configured, while the ouputs 2 to 4 are something different. Consult the
...
Is based on the _Dual bit-mangler_ mode of the _Parasite_ firmware for _Mutable Instruments Warps_.
...
Is based on the _Granular_ mode of the _Parasite_ firmware for _Mutable Instruments Clouds_.
...
Put simply, this uses a filter to split the input audio into the high and low parts. The low part is sent to a virtual box called drum and the high
part to so called horns. Both drum and horns rotate (independently from another), thus create the weird but pleasant tremolo-like sound.
